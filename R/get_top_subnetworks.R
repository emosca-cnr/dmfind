...
get_top_subnetworks <- function(G, ranked_list, idx=1:length(ranked_list)){
    ...
    stat_out <- data.frame(idx=idx_curr, n_v=idx_curr, n_e=0, d=0, 
                           n_comp=0, n_v_max=0, n_v_min=0, n_v_conn_tot=0)
    j <- 1
    for(i in idx_curr){
        g_out[[j]] <- 
            induced_subgraph(G, 
                             V(G)$name[V(G)$name %in% names(ranked_list)[1:i]])
        j <- j+1
    }
    ...
    stat_out$n_v_conn_tot <- 
        unlist(lapply(cl, function(x) sum(x$csize[x$csize>1])))
    stat_out$omega <- 
        unlist(lapply(idx_curr, function(x) 
            omega(G, ranked_list[1:x] / max(ranked_list[1:x]))[x]))
