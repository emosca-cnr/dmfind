...
#' @export
#'
check_eps <- function (X0, Xs, x = NA, eps = c(0.1, 0.25, 0.5, 1), top = 250, sort.X0=FALSE)
{
  ...
  for(i in 1:length(eps)){
    res[[i]] <- unlist(lapply(1:top, function(xx) sum(rownames(X0_sorted)[X0_sorted>0] %in% rownames(S_eps[[i]])[1:xx])/min(xx, N_pos_X0)))
  }
