...
extract_module <- function(graph, selected_vertices, X0=NULL, min_subnet_size = 2){

  #network between the selected gene
  dm <- igraph::induced_subgraph(graph, V(graph)$name[match(selected_vertices, V(graph)$name)])
  ...
  #plot
  dm_components <- igraph::induced_subgraph(dm, V(dm)$name[V(dm)$subnet_size >= min_subnet_size])
