...
NR <- function(G, ranked_vector, k=99, mc.cores=1, null_model=c("A", "S", "SA")) {
  ...
  omega_vect <- omega(G, ranked_vector_norm)
  ...
  if(mc.cores > 1){
    res <- parallel::mclapply(idx.perm, omega_perm, G=G, dS=ranked_vector_norm, mc.cores = mc.cores, null_model=null_model)
  }else{
    res <- lapply(idx.perm, omega_perm, G=G, dS=ranked_vector_norm, null_model=null_model)
  }
