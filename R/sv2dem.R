...
sv2dem <- function(X, G, mc.cores=2, NR_p_thr=0.05, NR_k=100, 
  ...
  sig_comp <- find_sign_conn_comp(NR_res$NR_summary, 
                                  NR_p_thr, min_rank, max_rank)
  ...
  settings <- list(mc.cores=mc.cores, NR_p_thr=NR_p_thr, NR_k=NR_k, 
                   min_module_size_final=min_module_size_final, 
                   min_rank=min_rank, max_rank=max_rank, 
                   NR_max_rank=NR_max_rank, min_subnet_size=min_subnet_size, 
                   plot_outfile=plot_outfile)

  return(list(smoothed_scores=smoothed_scores, NR_res=NR_res, 
              sig_comp=sig_comp, modules=modules, summary_log=summary_log, 
              settings=settings))
