...
#' @param top_network network with some parameters: "comm_id" for community membership and "label" for labels (for plotting purpose).
#' @param all_names TRUE/FALSE. Used to plot all gene names.
#' @param extreme_points TRUE/FALSE. Used to plot extreme poits of the plot, such as the gene with max and min participation coefficient and z-score.
#' @param choose_your_genes Vector of names (matching with "label" of network). Used to plot genes name of your choice.
...
pc_wmz <- function(top_network, all_names = FALSE, extreme_points = FALSE, choose_your_genes = NULL, out_file="pc_wmz.jpg", do_plot=TRUE) {
    ...
    df <- data.frame(name=names(pc), label=V(top_network)$label[match(names(pc), 
                    V(top_network)$name)], comm_id=V(top_network)$comm_id[match(names(pc),
                    V(top_network)$name)], pc=pc, wm_z=wm_z, stringsAsFactors = F)
  ...
  if(do_plot){
    if (all_names == TRUE & extreme_points == TRUE | all_names == TRUE & 
        extreme_points == TRUE & !is.null(choose_your_genes)) {
      ...
      if (all_names == TRUE) {
        warning("You have selected to plot all gene names. 
                To plot only expreme points use 'extreme_points = TRUE")
        v_labl <- V(top_network)$label
      }
      if (extreme_points == TRUE) {
        warning("You have selected to plot extreme gene names. 
                To plot all the gene names use 'all_names = TRUE'")

        name_max_zscore <- 
            V(top_network)$label[match(names(pc)[which.max(wm_z)], 
                                       V(top_network)$name)]

        # gene with max pc
        name_max_pc <- 
            V(top_network)$label[match(names(pc)[which.max(pc)], 
                                       V(top_network)$name)]

        # genes with pc = 0
        V(top_network)$label[match(names(pc)[pc == 0 & which.max(wm_z)], 
                                   V(top_network)$name)]

        # gene with min z score
        name_min_zscore <- 
            V(top_network)$label[match(names(pc)[which.min(wm_z)], 
                                       V(top_network)$name)]

        # gene with min pc
        name_min_pc <- 
            V(top_network)$label[match(names(pc)[which.min(pc)], 
                                       V(top_network)$name)]

        v_labl <- V(top_network)$label
        v_labl[!v_labl %in% c(name_max_zscore, 
                              name_max_pc, name_min_zscore, name_min_pc)] <- ""
      }
      ...
      if (all_names == FALSE & extreme_points == FALSE & is.null(choose_your_genes)) {
        warning(paste0("You don't have selected to plot gene names. 
                To plot only expreme points use 'extreme_points = TRUE' 
                and to plot all the gene names use 'all_names = TRUE'.
      ...
      text(c(0.03, 0.60, 0.78, 1.02, 0.28, 0.73, 1.02), 
           c(h_cut, h_cut, h_cut, h_cut, max_zscore+0.2, 
             max_zscore+0.2, max_zscore+0.2), paste0("R", 1:7), 
           cex=0.6, font=2, pos=1)

      if(any(v_labl!="")){
        plotrix::thigmophobe.labels(pc, wm_z, v_labl[match(names(pc), 
                                    V(top_network)$name)], cex=0.8)
      }
