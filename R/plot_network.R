...
plot_network <- function(graph, color_by=NULL, color_quant=TRUE, 
                         label_by="name", pal=NULL, plot_outfile="graph.jpg", 
                         community=NULL, comm_w_in=2, comm_w_b=1, lo=NULL, 
                         legend.off=FALSE, ...){
    ...
    if(!is.null(community)){
        ew <- edge_weights(community, graph, weight.within=comm_w_in,
                           weight.between=comm_w_b)
    }
    ...
    if(!is.null(color_by) & !legend.off){
        plot.new()
        legend("bottomright", levels(factor(color_values, 
              levels=sort(unique(color_values)))), 
