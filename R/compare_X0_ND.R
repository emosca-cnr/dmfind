...
compare_X0_ND <- function(G=NULL, X0_ranked_by_ND=NULL, X0_ranked_by_X0=NULL, norm=FALSE, do.plot=FALSE, file="./compare_X0_ND.jpg", ...){
  ...
  if(do.plot){
    ...
    plot(ans$omega_X0_X0, ylim=c(0, max(ans[, c("omega_X0_X0", "omega_X0")])), type="l", lty=2, lwd=2, xlab="rank", ylab="omega", main="")
    lines(ans$omega_X0, lwd=2)

    plot(ans$CCF_X0, ylim=c(0, max(ans[, c("CCF_X0", "CCF")])), type="l", lty=2, lwd=2, xlab="rank", ylab="CCF", main="")
    lines(ans$CCF, lwd=2)
