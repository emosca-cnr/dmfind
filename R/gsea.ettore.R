...
gsea <- function(rl=NULL, gsl=NULL, k=99, ord.mode=-1, mc_cores_path=1, mc_cores_perm=1, min.k.nes=10){
  ...
  for(i in 1:length(rll)){
    ...
    if(!is.numeric(n_nes_pos) | !is.numeric(n_nes_neg) | !is.numeric(nrow(nes))){
      print(i)
      print(nes)
