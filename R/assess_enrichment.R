...
assess_enrichment <- function(G=NULL, top_list=NULL, ranks=NULL, 
                              ranked_vector_X0=NULL, do.plot=FALSE, 
                              critical=NULL, file="top_net_enrichment.jpg"){

    ranks <- seq(10, length(top_list), by=10)
    gsea_res <- vector("list", length(ranks))
    net_ccf <- gsea_res
    for(i in 1:length(ranks)){
        net_ccf[[i]] <- get.n.conn.comp(induced_subgraph(G, 
                        V(G)$name[V(G)$name %in% top_list[1:ranks[i]]]), 2)

        ## if the sum of gene set scores is 0 we can not evaluate the enrichment
        if(sum(ranked_vector_X0[names(ranked_vector_X0) %in% 
                                V(net_ccf[[i]])$name])>0){
            gsea_res[[i]] <- gsea(matrix(ranked_vector_X0, ncol = 1, 
                                         dimnames = list(names(ranked_vector_X0), "X0")), 
                                         list(top_net=V(net_ccf[[i]])$name))
        }else{
            warning("No positive elments for ", ranks[i], "\n")
            gsea_res[[i]] <- list(gs_table=list(X0=data.frame(id=1, 
                                  es=0, p_val=1, adj_p_val=1, nes=0, FDRq=1, 
    ...
    if(do.plot){
        ...
        plot(gsea_res_df$id, gsea_res_df$size, pch=16, xlab="rank", 
        ...
        plot(gsea_res_df$id, log10(gsea_res_df$FDRq), pch=16, xlab="rank", 
        ...
        text(gsea_res_df$id[top_10_idx], log10(gsea_res_df$FDRq)[top_10_idx], 
             gsea_res_df$id[top_10_idx], cex=0.7, pos = 3)
