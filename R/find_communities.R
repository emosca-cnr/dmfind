...
#' @import igraph
#' @export
find_communities <- function(g, e.weights=NULL, v.weights=NULL, 
    ...
    if("fastgreedy" %in% methods){
        ...
        comm_info$modularity[idx] <- 
            igraph::modularity(g, comm_list[[idx]]$membership,
                               weights=e.weights)
    ...
    if("labprop" %in% methods){
        ...
        comm_list[[idx]] <- 
            igraph::label.propagation.community(g, weights=e.weights)
        comm_info$modularity[idx] <- 
            igraph::modularity(g, comm_list[[idx]]$membership, 
                               weights=e.weights)
        comm_info$n[idx] <- max(comm_list[[idx]]$membership)
    ...
    if("eigen" %in% methods){
        ...
        comm_list[[idx]] <- 
            igraph::leading.eigenvector.community(g, weights=e.weights)
        comm_info$modularity[idx] <- 
            igraph::modularity(g, comm_list[[idx]]$membership, 
                               weights=e.weights)
        comm_info$n[idx] <- max(comm_list[[idx]]$membership)
    ...
    if("multilev" %in% methods){
        ...
        comm_list[[idx]] <-
            igraph::multilevel.community(g, weights=e.weights)
        comm_info$modularity[idx] <- 
            igraph::modularity(g, comm_list[[idx]]$membership, 
                               weights=e.weights)
        comm_info$n[idx] <- max(comm_list[[idx]]$membership)
  ...
  if("infomap" %in% methods){
        ...
        comm_list[[idx]] <- 
            igraph::infomap.community(g, e.weights=e.weights, 
                                      v.weights=v.weights)
        comm_info$modularity[idx] <- 
            igraph::modularity(g, comm_list[[idx]]$membership, 
                               weights=e.weights)
