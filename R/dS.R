...
dS <- function(X0, Xs, classes, eps=1){
  ...
  dS_df <- data.frame(X0_1=as.numeric(X0_1), X0_2=as.numeric(X0_2),  dX0=as.numeric(X0_2 - X0_1), Xs_1=as.numeric(Xs_1), Xs_2=as.numeric(Xs_2), dXs=as.numeric(Xs_2 - Xs_1), S_1=as.numeric(nsi_1), S_2=as.numeric(nsi_1), dS=as.numeric(delta_S), row.names = rownames(X0), stringsAsFactors = FALSE)
