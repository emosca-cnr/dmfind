...
gsea <- function(rl, gsl, k=100, ord.mode=-1, 
                 mc_cores_path=1, mc_cores_perm=1){
    ...
    #real es
    print("ES...")
    #real_es <- lapply(gsl, function(x) lapply(rll, function(y) 
    #es(which(names(y) %in% x), y)))
    #real_es <- do.call(rbind, real_es)

    real_es_data <- lapply(gsl, function(x) 
        lapply(rll, function(y) es(which(names(y) %in% x), y, le=T)))
    real_es <- do.call(rbind, 
                       lapply(real_es_data, 
    ...
    for(i in 1:length(leading_edge)){
        leading_edge[[i]] <- 
            do.call(rbind, lapply(real_es_data, function(x) x[[i]]$lea))
    }
    ...
    if(mc_cores_path==1){
        if(mc_cores_perm == 1){
        #res <- lapply(gsl, function(x) unlist(lapply(x_perm, 
            ...
            #res <- lapply(gsl, function(x) unlist(mclapply(x_perm, 
            #function(y) calc_gs_perm(rl, y, x), mc.cores=mc_cores_perm)))
            res <- lapply(gsl, function(x) 
                    do.call(rbind, parallel::mclapply(x_perm, function(y) 
                        calc_gs_perm(rll, y, x), mc.cores=mc_cores_perm)))
        }
    }else{
        cat(length(gsl), "gene sets on", mc_cores_path, "cores\n")
        if(mc_cores_perm == 1){
            #res <- parallel::mclapply(gsl, function(x) 
            #unlist(lapply(x_perm, function(y) calc_gs_perm(rl, y, x))), 
            #mc.cores = mc_cores_path)
            res <- parallel::mclapply(gsl, function(x) 
                    do.call(rbind, lapply(x_perm, function(y) 
                    calc_gs_perm(rll, y, x))), mc.cores = mc_cores_path)
        }else{
            cat(k, "permutations on", mc_cores_perm, "cores\n")
            #res <- parallel::mclapply(gsl, function(x) 
            #unlist(parallel::mclapply(x_perm, function(y) 
            #calc_gs_perm(rl, y, x), mc.cores=mc_cores_perm)), 
            #mc.cores = mc_cores_path)
            res <- parallel::mclapply(gsl, function(x) 
                    do.call(rbind, parallel::mclapply(x_perm, function(y) 
                    calc_gs_perm(rll, y, x), mc.cores=mc_cores_perm)), 
    ...
    for(i in 1:length(rll)){
        p_val <- apply(res[[i]], 1, function(x) 
        ...
        fdrq <- sapply(nes[, 1], function(x) 
                ifelse(x>0, sum(all_nes >= x) / n_nes_pos, 
                sum(all_nes <= x) / n_nes_neg))
        fdrq <- fdrq / sapply(nes[, 1], function(x) 
                ifelse(x>0, sum(nes[, 1] >= x) / n_real_nes_pos, 
