...
scores <- merge(X0, resS$Sp, by=0, sort=FALSE, all.y=TRUE)
...
nr_res <- NR(g, sort(resS$Sp[, 1], decreasing = TRUE), k = 99,
          mc.cores = 2)
...
V(top_network)$label <- " "
dmfind::pc_wmz(top_network, do_plot=FALSE)
```